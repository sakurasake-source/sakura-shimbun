...
## Claude skills（流程编排靠这两个，不再靠临场回忆）

- `sakura-news-issue`：出一期日报（采集→选材→版面→渲染→发私密）
- `xhs-single-post`：单条搬运（下载→逐帧读字幕→文案→发私密）
（均在 `~/.claude/skills/`；「生成发帖包一条龙」由 skill 编排实现——文案是编辑判断，不硬脚本化）
